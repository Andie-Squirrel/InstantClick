...
try: # Default preinstalled modules whose availability depends on your operating system
	...
	import msvcrt # a better module to check for keypresses, but not to send them out, I think. It also adds weird a weird b'' to the character input
...
def checkEsc():
	# get the key
	if msvcrt.kbhit():
		# Convert it to a string (but without the thread)
		key = str(msvcrt.getch())
		# Trim off the unnecessary `b'` and `'` characters on either end of the output from getch
		key = key[2:-1] # removes 2 chars from the left and 1 char from the right
		if key == esc: # if the string matches the escape character for `ESC`, exit
			print('esc by msvcrt module')
			return True
	elif keyboard.is_pressed('esc'): # As a backup in case MSVCRT did not work, the `keyboard` module is used to detect an `ESC` press
